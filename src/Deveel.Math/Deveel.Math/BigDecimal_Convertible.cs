...
#endif
{
    ...
    /// </returns>
    public string ToEngineeringString() => ToEngineeringString(null);
    ...
    /// notation if necessary.
    /// </returns>
    public string ToEngineeringString(IFormatProvider provider) => DecimalString.ToEngineeringString(this, provider);
    ...
    public string ToPlainString() => ToPlainString(null);
...
*/
    ...
    public BigInteger ToBigIntegerExact()
    {
        ...
        else
        {
            ...
            integer = BigMath.DivideAndRemainder(GetUnscaledValue(), Multiplication.PowerOf10(_scale), out BigInteger fraction);
            if (fraction.Sign != 0)
            {
...
* If the scale is positive and very large the long value could be zero. */
        ((_scale <= -64) || (_scale > AproxPrecision()) ? 0L : ToBigInteger().ToInt64());
    ...
    public long ToInt64Exact() => ValueExact(64);
...
* If the scale is positive and very large the long value could be zero. */
        ((_scale <= -32) || (_scale > AproxPrecision()) ? 0 : ToBigInteger().ToInt32());
    ...
    public int ToInt32Exact() => (int)ValueExact(32);
    ...
    public short ToInt16Exact() => (short)ValueExact(16);
    ...
    public byte ToByteExact() => (byte)ValueExact(8);
    ...
    public double ToDouble()
    {
        ...
        {
            // mantisa = abs(u) * 10^s
            mantisa *= Multiplication.PowerOf10(-_scale);
        }
        else
        {
            // (scale > 0)
            ...
            quotient = BigMath.DivideAndRemainder(mantisa, powerOfTen, out BigInteger remainder);

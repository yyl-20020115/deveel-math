...
public static class BigDecimalMath
{
    ...
    {
        var quotient = BigMath.DivideAndRemainder(scaledDividend, scaledDivisor, out BigInteger remainder);
        if (remainder.Sign == 0)
        {
    ...
    public static BigDecimal Divide(BigDecimal dividend, BigDecimal divisor)
    {
        ...
        do
        {
            quotient = BigMath.DivideAndRemainder(q, BigDecimal.FivePow[i], out BigInteger remainder);
            if (remainder.Sign == 0)
            {
                l += i;
                if (i < lastPow)
    ...
    public static BigDecimal Divide(BigDecimal dividend, BigDecimal divisor, MathContext mc)
    {
        ...
        quotient = BigMath.DivideAndRemainder(quotient, divisor.UnscaledValue, out BigInteger remainder);
    ...
    public static BigDecimal DivideToIntegralValue(BigDecimal dividend, BigDecimal divisor)
    {
        ...
        else
        {
            ...
            {
                quotient = BigMath.DivideAndRemainder(integralValue, BigDecimal.TenPow[i], out BigInteger remainder);
                if ((remainder.Sign == 0)
    ...
    public static BigDecimal DivideToIntegralValue(BigDecimal dividend, BigDecimal divisor, MathContext mc)
    {
        ...
        else if (diffScale > 0)
        {
            ...
            quotient *= Multiplication.PowerOf10(newScale);
        }
        else
        {
            // CASE s2 > s1:   
        ...
        BigDecimal integralValue = new(quotient);
